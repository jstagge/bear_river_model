...
### Read in monthly USGS flow and fix data type
flow_file_name <- "little_bear_base_flows_by_forks.csv"
flow_by_forks <- read.csv(file.path(output_path,paste0("monthly_flow/",flow_file_name)))
...
###########################################################################
year_test <- flow_expand$year > 1960

new_fit <-  fit_wrapper(data = flow_expand[year_test,], predictors = c("Little_Bear", "EF_withdrwl_paradise_highline", "SF_withdrwl_hyrum_canal"), month_loop=seq(5,9), y_name="East_Fork")

### Insert data
estimate_all <- rep(NA, length(flow_expand$year))
estimate_all[year_test] <- new_fit$estimate
insert_test <- is.na(flow_expand$East_Fork) & !is.na(estimate_all)
flow_expand$East_Fork[insert_test] <- estimate_all[insert_test]
flow_expand$data_EF[insert_test] <- "estimated with LB EF_withd SF_withd"

### Test plot
plot(flow_expand$date, flow_expand$East_Fork, type="l")


new_fit <-  fit_wrapper(data = flow_expand[year_test,], predictors = c("South_Fork", "EF_withdrwl_paradise_highline", "SF_withdrwl_hyrum_canal"), month_loop=seq(5,9), y_name="East_Fork")
...
insert_test <- is.na(flow_expand$East_Fork) & !is.na(estimate_all)
flow_expand$East_Fork[insert_test] <- estimate_all[insert_test]
flow_expand$data_EF[insert_test] <- "estimated with SF EF_withd SF_withd"
...
sep_subset <- sep_subset[,c("water_year", "East_Fork")]
...
may_subset <- may_subset[,c("water_year", "East_Fork")]
...
new_fit <-  fit_wrapper(data = flow_expand, predictors = c("South_Fork", "Prev_Sep", "Future_May"), month_loop=c(seq(10,12), seq(1,4)), y_name="East_Fork")

### Insert data
insert_test <- is.na(flow_expand$East_Fork) & !is.na(new_fit$estimate)
flow_expand$East_Fork[insert_test] <- new_fit$estimate[insert_test]
flow_expand$data_SF[insert_test] <- "estimated with SF Prev_Sep Future_May"



### Fit using other branches and Prev/Future
new_fit <-  fit_wrapper(data = flow_expand[year_test,], predictors = c("Prev_Sep", "Future_May"), month_loop=c(seq(10,12), seq(1,4)), y_name="East_Fork")
...
insert_test <- is.na(flow_expand$East_Fork) & !is.na(estimate_all)
flow_expand$East_Fork[insert_test] <- estimate_all[insert_test]
flow_expand$data_EF[insert_test] <- "estimated with Prev_Sep Future_May"


plot(flow_expand$date, flow_expand$East_Fork, type="l")


plot(flow_expand$date[780:900], flow_expand$East_Fork[780:900], type="l")
lines(flow_expand$date, flow_expand$South_Fork, col="blue")
lines(flow_expand$date, flow_expand$Little_Bear, col="red")
lines(flow_expand$date, flow_expand$Prev_Sep, col="orange")
lines(flow_expand$date, flow_expand$Future_May, col="purple")
...
interp_ts <- linear_interp(ts=flow_expand$East_Fork, max_gap=6)
### Save results back to dataframe
flow_expand$East_Fork <- interp_ts$interp_vals
flow_expand$data_EF[interp_ts$interp_test] <- "interpolated"


###########################################################################
###  Re-insert data to original dataframe
###########################################################################
flow_by_forks$East_Fork <- flow_expand$East_Fork
flow_by_forks$data_EF <- flow_expand$data_EF
...
#saveRDS(model_result, file.path(write_output_path, "EF_model_fit.rds"))
...
### Save to pdf and png
ggsave(file.path(write_figure_path, "Little_Bear_ts_by_fork_EF_fixed.png"), p, width=8, height=4, dpi=600)
ggsave(file.path(write_figure_path, "Little_Bear_ts_by_fork_EF_fixed.pdf"), p, width=8, height=4)
